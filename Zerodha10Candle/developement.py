...
def start_excel():
    ...
    ttn = excel_name.sheets("Trade_Terminal_Normal")
    ...
    ttn.range(f"a1:t1").value = [ "Symbol", "Open", "High", "Low", "Close", "VWAP", "Best Buy Price",
                                "Best Sell Price","Volume", "LTP","Percentage change", "Qty", "Direction", "Entry Signal", "Exit Signal", "Entry Price",
                                "Exit Price", "Target","SL" ,"Trade Status"]
    ...
    while True:
        try:
            ...
            #ttn.range(f"a{2}:t{1000}").value = tt.range(f"a{2}:t{1000}").value
            main_list = []
            candle_list=[]
            candle_list_ha=[]
            ...
            for i in symbols:
                ...
                cand_lst= [None for _ in range(42)]
                cand_lst_ha= [None for _ in range(42)]
                if i:
                    ...
                    if i in subs_lst:
                        try:
                            ...
                            his_data= historicaldata.gethistoricaldata(kite,TokenKey,interval)
                            cand_lst = his_data["normal"]
                            cand_lst_ha = his_data["ha"]
                ...
                candle_list.append(cand_lst)
                candle_list_ha.append(cand_lst_ha)
                
                idx += 1

            tt.range("b2:k1000").value = main_list
            tt.range("v2:bk1000").value = candle_list_ha
            ttn.range("b2:k1000").value = main_list
            ttn.range("v2:bk1000").value = candle_list
...
if __name__ == '__main__':
    if(Zerodha_login() == 1 ):   
        try:
            ...
            """
            #print(ins_df)
            test =kite.historical_data(instrument_token=4514561,from_date="2023-10-10 09:20:00",to_date="2023-10-17 13:20:00",interval="5minute")
            #print(test)
            ...
            print(test2)
            test2.to_csv("zydus.csv",index=False)
